...
class DX_Auth
{
	...
	*/
	...
	function _email($to, $from, $subject, $message)
	{
		...
		$CI->email->from($from, 'Unravel Notify');
		$CI->email->to($to);
	...
	function _set_last_ip_and_last_login($user_id)
	{
		...
		{
			// Load model
			$this->ci->load->model('dx_auth/UserModel', 'users');
			// Update record
	...
	*/
	...
	// Get user id
	function get_profile_field($id, $field)
	{
		$this->ci->load->model('dx_auth/User_Profile', 'user_profile');
		return $this->ci->user_profile->get_profile_field($id, $field);
	...
	function is_role($roles = array(), $use_role_name = TRUE, $check_parent = TRUE)
	{
		...
		if ($use_role_name)
		{
			$check_array[] = $this->ci->session->userdata('DX_role_name');
		}
		else
		{
	...
	{		
		...
		{				
			...
			// Trigger event
			$this->ci->dx_auth_event->user_activated($this->ci->db->insert_id(), $username);				
		}
