...
function create_captcha($data = '', $img_path = '', $img_url = '', $font_path = '')
{
   ...
   if ($word == '')
   {
		...
		$pool = '23456789abcdefghikmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ';
	...
	if ( $show_grid == TRUE )
	{
		// X grid
		$grid = rand(12,22);
		for ($x = 0; $x < $img_width; $x += mt_rand($grid - 2, $grid + 3))
		{
		...
		{
			$current_colour = $grid_color[array_rand($grid_color)];
			imageline($im, mt_rand(4,8), mt_rand($y - 3, $y), mt_rand($img_width - 5, $img_width), mt_rand($y - 3, $y), $current_colour);
		}
	}
	...
	else
	{
		...
		$font_size	= !empty($font_size) ? $font_size : mt_rand(18,20);
	...
	$img = "<img src=\"$img_url$img_name\" width=\"$img_width\" height=\"$img_height\" class='captchaImage' style=\"border:0;\" alt=\" \" />";
